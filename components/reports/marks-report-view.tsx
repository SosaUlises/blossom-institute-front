...
import { getMarksExportExcelUrl, getMarksExportPdfUrl, getMarksReport } from '@/lib/reports/api'
import type { CursoListItem } from '@/lib/courses/types'
import type { ReporteMarksResponse } from '@/lib/reports/types'
      ...
      {report && (
        <>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <Card className="border-border/70 bg-card/95">
              <CardContent className="p-5">
                <p className="text-xs uppercase tracking-wide text-muted-foreground">Promedio general</p>
                <p className="mt-2 text-2xl font-bold text-foreground">
                  {report.resumen.promedioGeneralCurso?.toFixed(2) ?? '-'}
                </p>
              </CardContent>
            </Card>
          </div>
                ...
                <table className="w-full min-w-[980px] text-sm">
                  <thead className="border-b border-border/70 bg-muted/30">
                    <tr className="text-left text-muted-foreground">
                      ...
                      <th className="px-5 py-4 font-medium">Prom. quiz</th>
                      <th className="px-5 py-4 font-medium">Tests</th>
                      <th className="px-5 py-4 font-medium">Prom. test</th>
                      <th className="px-5 py-4 font-medium">Total marks</th>
                      <th className="px-5 py-4 font-medium">Prom. general</th>
                    </tr>
                  </thead>
                    ...
                    {report.items.length === 0 ? (
                      ...
                      report.items.map((item) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-4 text-muted-foreground">{item.quizCount}</td>
                          <td className="px-5 py-4 text-muted-foreground">
                            {item.quizPromedio?.toFixed(2) ?? '-'}
                          </td>
                          <td className="px-5 py-4 text-muted-foreground">{item.testCount}</td>
                          <td className="px-5 py-4 text-muted-foreground">
                            {item.testPromedio?.toFixed(2) ?? '-'}
                          </td>
                          <td className="px-5 py-4 text-muted-foreground">{item.marksCount}</td>
