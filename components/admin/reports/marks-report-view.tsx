...
import {
  getMarksExportExcelUrl,
  getMarksExportPdfUrl,
  getMarksReport,
} from '@/lib/admin/reports/api'
import type { CursoAlumno } from '@/lib/admin/courses/people-types'
...
function buildStudentAvatarMap(students?: CursoAlumno[]) {
  return new Map(
    (students ?? []).map((student) => [student.alumnoId, student.avatarUrl])
  )
}
...
export function MarksReportView() {
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      const studentAvatarMap = buildStudentAvatarMap(courseStudents?.items)
      ...
      setReport({
        ...
        items: data.items.map((item) => ({
          ...
          alumnoAvatarUrl:
            ...
            item.avatarUrl ??
            studentAvatarMap.get(item.alumnoId) ??
            null,
        })),
      })
    } catch (err: any) {
  ...
  return (
    <CourseReportHero
      ...
      description="Analizá el rendimiento de un curso por año y trimestre, con exportación en Excel o PDF."
      context={
        <ReportHeroContext
          items={[
            {
    ...
    >
      <ReportFilterPanel
        description="Seleccioná curso, año, trimestre y búsqueda opcional para generar el análisis."
        error={error}
        action={
        ...
        <>
          <ReportSummarySection description="Indicadores del reporte">
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              />
              <SummaryCard
                title="Calificaciones analizadas"
                value={report.resumen.totalMarks}
                icon={BarChart3}
                accent="emerald"
                helper="Registros evaluados en el período."
              />
              <SummaryCard
                ...
                icon={Sigma}
                accent="amber"
                helper="Resultado global del período."
                highlight
              />
            </div>
          </ReportSummarySection>
          ...
          <ReportResultsSection
            ...
            description="Vista consolidada del rendimiento individual para el período seleccionado."
          >
                ...
                <tbody>
                  {report.items.length === 0 ? (
                    <ReportEmptyTableRow
                      ...
                      description="No encontramos alumnos con calificaciones para este curso y período."
                    />
                  ) : (
                        ...
                        <td className="px-6 py-5">
                          <span
                            className={cn(
                              ...
                              getAverageTone(item.promedioGeneral),
                            )}
                          >
            ...
            <div className="grid w-full gap-2 sm:grid-cols-2 sm:min-w-[260px]">
              <ReportExportButton
                ...
                filename={buildReportFilename(
                  ['calificaciones', report.resumen.cursoNombre, report.resumen.year, `t${report.resumen.term}`],
                  'xlsx'
                )}
                href={
                  report && cursoId
