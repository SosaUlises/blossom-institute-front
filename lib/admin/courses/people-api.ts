...
export async function getCursoAlumnos(cursoId: number) {
  const res = await fetch(`/api/admin/courses/${cursoId}/alumnos`, {
    credentials: 'include',
    cache: 'no-store',
  })
...
export async function removeAlumno(cursoId: number, alumnoId: number) {
  const res = await fetch(`/api/admin/courses/${cursoId}/alumnos/${alumnoId}`, {
    method: 'DELETE',
    credentials: 'include',
  })
...
export async function getCursoProfesores(cursoId: number) {
  const res = await fetch(`/api/admin/courses/${cursoId}/profesores`, {
    credentials: 'include',
    cache: 'no-store',
  })
...
export async function removeProfesor(cursoId: number, profesorId: number) {
  const res = await fetch(`/api/admin/courses/${cursoId}/profesores/${profesorId}`, {
    method: 'DELETE',
    credentials: 'include',
  })
